...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if((r=envid2env(envid,&e,1))<0)
		return r;
	e->env_tf=*tf;
	e->env_tf.tf_eflags=(e->env_tf.tf_eflags&(~FL_IOPL_MASK))|FL_IF;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	src_page=page_lookup(srce->env_pgdir,srcva,&src_pte);
	if(src_page==NULL){
		res=-E_INVAL;
		return res;
	}
	...
	if(((*src_pte&PTE_W)==0)&&(perm&PTE_W)){
		res=-E_INVAL;
		return res;
	}
	res=page_insert(dste->env_pgdir,src_page,dstva,perm);
	return res;	
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	pte_t *tmp;
	void *dstva;
	if((r=envid2env(envid,&rec,0))<0)
		return r;
	if((rec->env_ipc_recving==0)||(rec->env_status!=ENV_NOT_RUNNABLE))
	...
	if((uint32_t)srcva<UTOP){
		...
		if(((uint32_t)dstva<UTOP)&&((uint32_t)dstva%PGSIZE==0)){
			pg=page_lookup(curenv->env_pgdir,srcva,&tmp);
			if((r=page_insert(rec->env_pgdir,pg,dstva,perm))<0){
				return -E_NO_MEM;
			}
			rec->env_ipc_perm=perm;
		}
	}
	rec->env_ipc_recving=0;
	rec->env_ipc_from=curenv->env_id;
	rec->env_ipc_value=value;
	rec->env_status=ENV_RUNNABLE;
	rec->env_tf.tf_regs.reg_eax=0;
	//sched_yield();
	return 0;
	//rec continue to run with retrun value 0
...
// Transmit packet by e1000
int sys_transmit_packet(void *va , uint16_t length){
	user_mem_assert(curenv,va,length,0);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_ipc_try_send:
			res=sys_ipc_try_send((envid_t)a1,a2,(void *)a3,(unsigned int)a4);
			break;
		case SYS_ipc_recv:
